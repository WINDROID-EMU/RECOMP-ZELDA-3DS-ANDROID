...
constexpr uint32_t kState1KnockbackRecoil = 0x04000000U;
...
constexpr uint32_t kActorWallPolyOffset = 0x0078U;
...
constexpr uint32_t kPlayerLedgeClimbTypeOffset = 0x2278U;
constexpr uint16_t kBgCheckFlagGround = 0x0001U;
constexpr uint16_t kBgCheckFlagWall = 0x0008U;
constexpr uint16_t kBgCheckFlagPlayerWallInteract = 0x0200U;
...
bool PlayerSprintRuntime::IsClimbingOrHanging(uint32_t stateFlags1,
                                              uint8_t ledgeClimbType,
                                              uint16_t bgCheckFlags,
                                              uint32_t actionFunc,
                                              uint32_t animIndex) noexcept {
    if (IsClimbingOrHanging(stateFlags1)) {
        return true;
    }
    if (ledgeClimbType != 0U) {
        return true;
    }
    if ((bgCheckFlags & kBgCheckFlagPlayerWallInteract) != 0U) {
        return true;
    }
    // Ações explícitas de escalada de superfícies (escadas, trepadeiras, vinhas, muros, beiradas)
    if (actionFunc == 0x004BE20CU || actionFunc == 0x004BEB58U ||
        actionFunc == 0x004BEBF4U || actionFunc == 0x004BEE00U ||
        actionFunc == 0x004BEE78U || actionFunc == 0x004C11A0U ||
        actionFunc == 0x00495E1CU || actionFunc == 0x00496458U ||
        actionFunc == 0x00495FD8U) {
        return true;
    }
    // Animações de escalada (incluindo escalada lateral em vinhas/hera: 0x11C e 0x120)
    if ((animIndex >= 0x0104U && animIndex <= 0x0130U) ||
        animIndex == 0x00ADU || animIndex == 0x00AEU) {
        return true;
    }
    return false;
}
...
PlayerSprintStatus PlayerSprintRuntime::Update(bool aButtonHeld,
    ...
    const bool climbingOrHanging = IsClimbingOrHanging(stateFlags1) || isClimbing;
    ...
    const bool disabled = (stateFlags1 & (kState1Paralyzed | kState1RidingHorse | kState1KnockbackRecoil)) != 0U ||
    ...
    if (invalidState || hitWall) {
        if (mState == PlayerSprintState::RollWaiting || mState == PlayerSprintState::Sprinting) {
            SPRINT_LOG("Sprint invalidated: grounded=%d, jumping=%d, pushing=%d, climbHang=%d, dialogue=%d, hitWall=%d, flags1=0x%08X",
                       (int)isGrounded, (int)jumpingOrAirborne, (int)pushingBoxes, (int)climbingOrHanging, (int)inDialogueOrCs, (int)hitWall, stateFlags1);
            mState = PlayerSprintState::Idle;
    ...
    case PlayerSprintState::Idle: {
        ...
        // Se o botão A foi pressionado durante movimento no chão, sem colisão e sem estados inválidos,
        // inicia o monitoramento do rolamento
        if (!invalidState && !hitWall && justPressedA && hasMovementIntent && mStaminaRemaining > 1.0f) {
    ...
    case PlayerSprintState::RollWaiting: {
        ...
        // ou se colidiu com parede/obstáculo, ou se a velocidade linear recuou/parou contra algo (rebound/bonk)
        const bool bonkedOrStopped = hitWall || (mRollTimer > 0.1f && (speedXZ <= 0.05f || linearVelocity <= 0.0f));

        if (!aButtonHeld || invalidState || bonkedOrStopped) {
            SPRINT_LOG("RollWaiting cancelled: aButtonHeld=%d, invalidState=%d, hitWall=%d, bonked=%d, rollTimer=%.2fs",
                       (int)aButtonHeld, (int)invalidState, (int)hitWall, (int)bonkedOrStopped, mRollTimer);
            mState = PlayerSprintState::Idle;
            mRollTimer = 0.0f;
        } else if (mRollTimer >= mConfig.RollDurationSeconds) {
            // O rolamento terminou e o Link se levantou;
            // SÓ inicia a corrida se Link estiver se movendo para frente normalmente e NÃO bateu em nada
            const bool isMovingForward = speedXZ > 1.0f && linearVelocity > 0.5f;
            if (!hitWall && isMovingForward && hasMovementIntent && mStaminaRemaining > 0.0f) {
                ...
                mState = PlayerSprintState::Idle;
                SPRINT_LOG("Roll finished but cannot sprint (hitWall=%d, movingFwd=%d, moveIntent=%d) -> Returning to Idle.",
                           (int)hitWall, (int)isMovingForward, (int)hasMovementIntent);
            }
            mRollTimer = 0.0f;
        }
        break;
    ...
    case PlayerSprintState::Sprinting: {
        // Condições para interromper a corrida (incluindo colisão com parede/obstáculo)
        if (!aButtonHeld || !hasMovementIntent || invalidState || hitWall) {
            SPRINT_LOG("Sprint ending: aHeld=%d, hasMoveIntent=%d, invalid=%d, hitWall=%d",
                       (int)aButtonHeld, (int)hasMovementIntent, (int)invalidState, (int)hitWall);
            mState = PlayerSprintState::Idle;
            mSpeedMultiplier = 1.0f; // Para a velocidade na hora
            mSprintTimer = 0.0f;
        } else {
    ...
    // Gerenciamento da velocidade: se estiver em estado inválido ou bater na parede, para imediatamente ("na hora")
    if (mState == PlayerSprintState::Sprinting) {
        ...
        if (invalidState || hitWall) {
            // Em qualquer estado inválido ou colisão, a velocidade para IMEDIATAMENTE na hora
            mSpeedMultiplier = 1.0f;
        } else {
...
bool ApplyGuestPlayerSprint(
    ...
    uint32_t wallPoly = 0;
    memory.Read32(player + kActorWallPolyOffset, &wallPoly);
    ...
    const bool hitWall = ((bgCheckFlags & kBgCheckFlagWall) != 0U) || (wallPoly != 0U);

    uint32_t actionFunc = 0;
    memory.Read32(player + kPlayerActionFunctionOffset, &actionFunc);

    uint8_t ledgeClimbType = 0;
    memory.Read8(player + kPlayerLedgeClimbTypeOffset, &ledgeClimbType);

    uint32_t animIndex = 0;
    memory.Read32(player + 0x0284U, &animIndex);

    // Para ser considerado escalando ou pendurado:
    // Verifica flags de estado, tipo de escalada de borda, flag de interação com parede,
    // ponteiro de função de ação (climb) e índice de animação (subida/lateral em vinhas/paredes).
    const bool isClimbingOrHanging = PlayerSprintRuntime::IsClimbingOrHanging(
        stateFlags1, ledgeClimbType, bgCheckFlags, actionFunc, animIndex);
    ...
    if (status.IsSprinting && isGrounded && !isClimbingOrHanging && !hitWall) {
        // Só força linearVelocity e speedXZ se Link realmente estiver em movimento para frente
        // (currentLinVel > 0.5f e speedXZ > 0.5f), nunca se estiver parado, recuando ou sentado no chão
        if (currentLinVel > 0.5f && currentLinVel < 5.66f) {
    ...
    if (status.SpeedMultiplier > 1.0f && status.IsSprinting && isGrounded && !isClimbingOrHanging && !hitWall) {
